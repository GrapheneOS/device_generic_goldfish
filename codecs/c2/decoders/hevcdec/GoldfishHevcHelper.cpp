...
void HevcTraits::createDecoder(iv_obj_t *&decHandle, IV_COLOR_FORMAT_T colorFormat, int numCores, int &stride) {
...
void HevcTraits::destroyDecoder(iv_obj_t *decHandle) {
    if (decHandle) {
...
void HevcTraits::setParams(iv_obj_t *decHandle, size_t stride, IVD_VIDEO_DECODE_MODE_T dec_mode) {
    ...
    ivd_ctl_set_config_op_t *ps_set_dyn_params_op = &s_hevcd_set_dyn_params_op.s_ivd_ctl_set_config_op_t;
...
void HevcTraits::resetDecoder(iv_obj_t *decHandle) {
...
void HevcTraits::setNumCores(iv_obj_t *decHandle, int numCores) {
...
IV_API_CALL_STATUS_T HevcTraits::callApi(iv_obj_t *decHandle, void *ip, void *op) {
    return ivdec_api_function(decHandle, ip, op);
}

bool HevcTraits::isKeyFrame(const uint8_t *frame, int inSize) {
    ...
    if (frame[0] == 0 && frame[1] == 0 && frame[2] == 0 && frame[3] == 1) {
        ...
        if (naluType == 32 || naluType == 33 || naluType == 34) return true;
    }
    return false;
}
